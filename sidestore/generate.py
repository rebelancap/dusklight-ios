...
def gh(url):
    hdr = {"Accept": "application/vnd.github+json", "User-Agent": "dusklight-ports-source"}
    if TOKEN:
...
def ipa_version(url, fallback):
    ...
    try:
        with urllib.request.urlopen(urllib.request.Request(url, headers={"User-Agent": "dusklight-ports-source"})) as r:
